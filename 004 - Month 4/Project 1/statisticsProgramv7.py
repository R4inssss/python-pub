...
class Scattered:
    def __init__(self):
        self.x = None
        print('What are your x variables?')
        x = list(map(float, input('>>> ').split()))
        print('What are your y variables?')
        y = list(map(float, input('>>> ').split()))

        if len(x) != len(y):
            print('You must have the same number of variables.')
            return
...
class Arr:
    def __init__(self):
        print('What are your n, sum x, sum of square x, sum of y, sum of square y, and sum of xy values?')
        n, sumx, sumx2, sumy, sumy2, sumxy = map(float, input('>>> ').split())
        n = int(n)
        r = calculate_r(n, sumx, sumx2, sumy, sumy2, sumxy)
        print(f'Your r is: {r:.4f}')
        if r > 0:
            print('As x increases, y tends to increase (positive correlation).')
        elif r < 0:
