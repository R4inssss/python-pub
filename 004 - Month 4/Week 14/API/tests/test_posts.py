...
@pytest.mark.parametrize("title, content, published", [
    ("WEEEE", "WEWEEEEEEEEE", True),
    ("123", "456", True),
    ("Is this published", "or is it not", False),
])
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title": "123", "content": "456"})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "123"
    assert created_post.content == "456"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']


def test_unauthorized_user_create_post(client, test_posts):
    res = client.post("/posts/", json={"title": "123", "content": "456"})
    assert res.status_code == 401
...
def test_update_posts_non_exist(authorized_client, test_user, test_posts):
    data = {
        "title": "123",
        "content": "456",
        "id": test_posts[3].id
